...
def main():
    num_passwords = int(input("How many passwords do you wish to generate? "))
    print("Generating " + str(num_passwords) + " number of passwords")

    password_lengths = []

    print("Minimum length of password must be 3")

    for i in range(num_passwords):
        length = int(input("Enter the length of Password[s] @" + str(i + 1) + " "))
        if length < 3:
            length = 3
    ...
    for i, password in enumerate(passwords):
        print("Generated Password @" + str(i + 1) + " = " + password)
